...
public abstract class DecorationItemMixin 
{
	...
	private Optional<PaintingEntity> filterPlacedPainting(World world, BlockPos pos, Direction facing, Operation<Optional<PaintingEntity>> original, ItemUsageContext context) {
		...
		RegistryEntry<PaintingVariant> variantEntry = PaintStackUtil.GetVariantEntry(context.getStack());
			...
			if (result.isEmpty() && player != null) {
				player.sendMessage(
					PaintTextUtil.ServersideTranslatable("painting.invalid_space",
						PaintTextUtil.TranslatableVariantName(variantEntry.getIdAsString()).formatted(Formatting.YELLOW),
						Text.translatable("painting.dimensions", variantEntry.value().width(), variantEntry.value().height())
					),
					true
				);
			}
